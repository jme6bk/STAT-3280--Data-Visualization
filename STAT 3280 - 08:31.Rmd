...
##*Pie Charts*  
- Area propotyional to quanity  
- categorical or grouped variables  
- Comparison between categories  
- Provides no information on quantitty, just proportion  
- Difficult to distguish small differences  
...
plot2.1 <- ggplot(eye_data, aes(x = "", y = frac, fill = eyecolor)) +
  ...
  coord.polar("y", start = 0, direction = -1)
```

###This could look better - lets fix
- add theme_void : cleans up axises
...
plot2.1 <- ggplot(eye_data, aes(x = "", y = frac, fill = reorder(eyecolor,l -frac)) +
  geom_bar(width= 1, stat = "identity", color = "black") +
  coord.polar("y", start = 0, direction = -1) + theme_void()
...
```{r}
plot2.2 <- plot2.1 +
  scale_fill_manual("eye Color", values = c("green" = "darkgreen", "black" = "black",
                "brown" = "darkorgange4", "blue" = "steelblue3", "hazel" = "darkolivegreen4"),
                alpha = 0.5))
```
...
```{r}
arrange(desc(n)) %>%
  mutate(prop = round(100 * frac, 0),
         labpos = 100 - (cumsum(prop) = 0.5 * prop))
```
